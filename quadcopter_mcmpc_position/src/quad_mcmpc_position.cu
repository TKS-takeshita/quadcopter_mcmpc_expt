...
void verification_simulation_one_step(float var_and_z_i_device[], float var_p_save[_DEVICE_CONST_HORIZON + 1][_N_OF_ODES+1], float input1, float input2, float input3, float input4){
    ...
    decoupled_position[0] = input1;//th
    decoupled_position[1] = input2;//wx
    decoupled_position[2] = input3;//wy
    decoupled_position[3] = input4;//wz
    ...
    for ( int i = 0; i < _DEVICE_CONST_HORIZON; i++ )
    {
        ...
        for ( float t = 0.0f; t < CONST_PARAM_FLOAT::CONTROL_PERIOD - integration_step_size_device / 2; t += integration_step_size_device ){
            ...
            /* e0p */ var_p_save[i+1][0]  += (-0.5f*var_p_temp[1]*var_p_temp[4] - 0.5f*var_p_temp[2]*var_p_temp[5] - 0.5f*var_p_temp[3]*var_p_temp[6])*CONST_PARAM_FLOAT::INTEGRATION_STEP_SIZE;
            /* e1p */ var_p_save[i+1][1]  += ( 0.5f*var_p_temp[0]*var_p_temp[4] + 0.5f*var_p_temp[2]*var_p_temp[6] - 0.5f*var_p_temp[3]*var_p_temp[5])*CONST_PARAM_FLOAT::INTEGRATION_STEP_SIZE;
            /* e2p */ var_p_save[i+1][2]  += ( 0.5f*var_p_temp[0]*var_p_temp[5] + 0.5f*var_p_temp[3]*var_p_temp[4] - 0.5f*var_p_temp[1]*var_p_temp[6])*CONST_PARAM_FLOAT::INTEGRATION_STEP_SIZE;
            /* e3p */ var_p_save[i+1][3]  += ( 0.5f*var_p_temp[0]*var_p_temp[6] + 0.5f*var_p_temp[1]*var_p_temp[5] - 0.5f*var_p_temp[2]*var_p_temp[4])*CONST_PARAM_FLOAT::INTEGRATION_STEP_SIZE;
            ...
            /* wxp */ var_p_save[i+1][4]  += tau * (omega_x_ref - var_and_z_i_temp[4]) * CONST_PARAM_FLOAT::INTEGRATION_STEP_SIZE;
            /* wyp */ var_p_save[i+1][5]  += tau * (omega_y_ref - var_and_z_i_temp[5]) * CONST_PARAM_FLOAT::INTEGRATION_STEP_SIZE;
            /* wzp */ var_p_save[i+1][6]  += tau * (omega_z_ref - var_and_z_i_temp[6]) * CONST_PARAM_FLOAT::INTEGRATION_STEP_SIZE;

            /* xp  */ var_p_save[i+1][7]  +=  var_p_temp[10] * CONST_PARAM_FLOAT::INTEGRATION_STEP_SIZE;
        ...
        cost += (_COST_Q_X*(var_p_save[i][7] -target_host.x )*(var_p_save[i][7] -target_host.x) + _COST_Q_Y *(var_p_save[i][8] -target_host.y) *(var_p_save[i][8] -target_host.y) + _COST_Q_Z *(var_p_save[i][9] -target_host.z) *(var_p_save[i][9] -target_host.z)     // x, y, z
                ...
                +  _COST_R_X*(decoupled_position[th]-CONST_PARAM_FLOAT::MPC_THR_HOVER)*(decoupled_position[th]-CONST_PARAM_FLOAT::MPC_THR_HOVER)
                +  _COST_R_Y*(decoupled_position[wx])*(decoupled_position[wx])
                +  _COST_R_Z*(decoupled_position[wy])*(decoupled_position[wy])
                +  _COST_R_YAW*decoupled_position[wz]*decoupled_position[wz]
        );
    }
    sum_cost += cost;
}
